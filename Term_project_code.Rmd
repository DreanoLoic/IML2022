...
# And it works! I didn't know how to use your other loop though for mean CV perplexity!
  df_DummyCV <- data.frame(matrix(ncol = 2, nrow = 0))
  for (i in 1:10){   
    ind.test = split[[i]]
    ind.all = unlist(split)
    ind.train = ind.all[!ind.all %in% ind.test]
    ...
    new <- c(Dummy_accuracy(y, train_main_value), train_main_value)
    df_DummyCV <- rbind(df_DummyCV, new)
  }
  colnames(df_DummyCV) <-c("phat", "tmv")
  as.numeric(df_DummyCV$phat)
...
To compare the results with logistic regression with Lasso we train also logistic regression on the training data with all the 26 variables.
...
After selecting the model we trained it using the original, cleaned training data (npf_train.csv). The estimates of the probabilities of the observations in testing data (npf_hidden.csv) to be classified as "event" were predicted. If the probability is more than 0.5 the observation is labeled as "event" and otherwise "nonevent". The type of the event was set to be the same as the type occured most often in the training data. That is the type "II" with the proportion of 48,7 % the others being 14,7 % for the type "Ia" and 36,6 % for type "Ib".
...
x.ans <- as.matrix(npf.clean)
y.ans <- npf.all.clean$class2
x.ans.test <- as.matrix(npf_hidden_clean[,-1]) 

#cv.lambda.ans <- cv.glmnet(x.ans, y.ans, family = "binomial", alpha = 1)$lambda.min
answer.model <- glmnet(x.ans, y.ans, family = "binomial", alpha = 1, lambda = 0.09)
...
#CV Accuracy should be the same already calculated - it should be added at the beginning of the data frame "answer"
