...
RF.reg.CV= function(X,nm.cl,iteration,nb.grp,mtry,ntree){
  ###iteration : nombre de fois que la CV est realiser	
  ###nb.grp :  nbr de fold pour la CV
  ###X : matrice de descripteurs 
  ###Y : vecteur qui contient la variable a predire
  ###nm.cl :  vecteur de classe 
  
  ###Pouc chq CV, on va calculer le taux de bien predit sur l'ech a predire.
  ...
  for(it in 1:iteration){
    ...
    Yobs = NULL
    Ypred=NULL
    # list of the sub-samples
    ech.list = split.CV(Mat = X,nbG = nb.grp)
    for(i in 1:nb.grp){
      #choisi les nb.grp-1 echantillons pour l apprentissage
      ...
      X1 = X[list.mol,]
      Y = as.numeric(nm.cl[list.mol])
      ###creation de la matrice a predire
      Matpred = cbind(X1,Y)
      colnames(Matpred) = c(colnames(X1), "pred")
      #calcul du modele
      modele=randomForest(pred ~ ., data = Matpred,mtry=mtry,ntree=ntree)
      ###creation of validation set
      new.data=X[ech.list[[i]],]
      Ypred = append(Ypred,predict(modele,newdata=new.data))
      Yobs = append(Yobs,nm.cl[ech.list[[i]]])
    }   ###fin de la boucle de cross-validation
    #compute well-predicted rate for the CV test set
    #Tx.good = sum(diag(table(Ypred,Yobs)))/length(Ypred)
    #res.tx = c(res.tx,Tx.good)
    
    correl = cor(Yobs,Ypred)
    R2 = summary(lm(Ypred~Yobs))$r.squared
    perf  = cbind(correl,R2)
    CV.perf = rbind(CV.perf,perf)
  }
  return(apply(CV.perf,2,mean))
}
npf_train$class2=class2_num_train
rf.npf <- randomForest(class2~., data = npf_train, mtry = 5, importance = TRUE)
rf.npf2 <- randomForest(class2~., data = npf_train, mtry = 5, importance = TRUE)
RF.CV = RF.reg.CV(X= npf_train[,-1],nm.cl = class2_num_train,iteration = 20,nb.grp = 5,mtry = 5,ntree = 50)
split = split.CV(npf.all.clean,10)
phat_NBCV <- c()
for (i in 1:10){   
  ind.test = split[[i]]
