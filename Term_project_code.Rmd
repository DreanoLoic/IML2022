...
table(ifelse(phat_RF_train >= 0.5, 1, 0), npf_train$class2)
```
...
table(ifelse(phat_RF >= 0.5, 1, 0), npf_valid$class2)
```
...
## Option 2 with color -> info training and validation sets seems to cover the same space
