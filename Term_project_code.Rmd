...
The highest accuracy is received when using 5 first PCs so Naive Bayes when using the first 5 principal components to reduce the dimensionality is added to be one of the methods.
...
--- this is not updated: the data that is used is still the old "trimmed", not the cleaned data
```{r, echo=FALSE}
library(class)
set.seed(42)
...
----This model was mentioned in lectures in relation to npf data example so do we want to add it?
