...
npf_valid.rf <- data.frame(scale(npf_valid[,-1]),class2=npf_valid$class2)
...
kable(table(ifelse(phat_RF >= 0.5, 1, 0), npf_valid.rf$class2))
```
...
l.acc.RF_CV = NULL
l.perp.RF_CV = NULL
npf.all.clean.rf <- data.frame(scale(npf.all.clean[,-1]),class2=npf.all.clean$class2)
...
Different values of k are tried for for choosing the optimal k. Accuracy on the validation set for each of the selected k are calculated and are respectively 
