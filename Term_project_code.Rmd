...
--- in case of classification tree the number of variables should be 26^0.5 and in regression 26/3 => I suppose we have regression now when using dummy response? That's why I changed the number from 5 to 9. Fix the text!!!
...
df13=getTree(rf.npf,1,labelVar = T))
 
mygraph = graph_from_data_frame(df13)

ggraph(mygraph, circular = FALSE) + 
  geom_edge_elbow() +
  geom_node_point() +
  geom_node_point(color = "black", size = 12, show.legend = FALSE, family = "serif") +
  geom_node_text(aes(label = Gini),  colour = 'red', size=8,
...
for (l in seq(CV.rep)){
  ...
  for (i in 1:10){   
    ...
    phat_DummyCV[i] <- Dummy_accuracy(y, train_main_value)
  }
  phat_DummyCV <- mean(phat_DummyCV)
  l.acc.DummyCV = c(l.acc.DummyCV,phat_DummyCV)
  l.perp.DummyCV = c(l.perp.DummyCV)
}
...
for (l in seq(CV.rep)){
  ...
  for (i in 1:10){   
    ...
    ind.all = unlist(split)   
    ind.train = ind.all[!ind.all %in% ind.test]
    mod <- glm(class2 ~ ., npf.all.clean[ind.train, ], family = "binomial")
    phat_LGCV[ind.test]<- predict(mod, newdata = npf.all.clean[ind.test,], type = "response")
  }
...
4) k-NN
K nearest neighbor is tried with different values of k: 1, 5, 10, 15, 20 and 50. Accuracy on the  validation set for each of them are respectively
...
perplexity(p.knn_train,train.Y)
...
p <- answer.model %>% predict(newx = x.ans.test)
class4 <- ifelse(p>0.5,"event", "nonevent")

answer <- data.frame(class4, p)
colnames(answer) <- c("class4", "p")
#CV Accuracy should be calculated and added at the beginning of the data frame "aswer"

#write.csv(answer, "C:\\...path name...\\answer.csv", row.names = FALSE)
