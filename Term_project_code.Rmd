...
npf_hidden <- read.csv("npf_test.csv")
...
npf_hidden$class2 <- factor("event_test", levels = c("nonevent_test", "event_test"))
npf_hidden$class2[npf_hidden$class4 =="nonevent"] <- "nonevent_test"

# reodering of npf_hidden to have the classes column at the beginning (easier to remove them after) and removing the noninformatic columns
npf_hidden = npf_hidden[ ,c(3,105,5:104)]
```

```{r, message=FALSE, eval=TRUE,echo=FALSE,warning=FALSE}
#npf_pca <- rbind(npf.var[,-c(1)],  cbind("class2"=NA,npf_hidden[,-c(1:4)]))
npf_pca <- rbind(npf.var[,-c(1)], npf_hidden[,-c(1)])
...
lines(PVE,type="b",col=cols3[2],lty="dashed",pch=3)
legend("right",c("cumulative PVE","PVE"),col=cols3[1:2],lty=c("solid","dashed"),pch=c(1,3))
}
plot_PVE(pca.npf)
...
For the original data we would need around ${10}$ PCs to explain most of the variance.
...
npf_hidden_clean <- npf_hidden[,colnames(npf_hidden) %in% c(columns,"class2")]
...
npf_train.rf <- data.frame(scale(npf_train[,-1]),class2=npf_train$class2)
npf_train.rf$class2=class2_num_train
npf_valid.rf <- data.frame(scale(npf_valid[,-1]),class2=class2_num)
...
```{r, message=FALSE,warning=FALSE, echo=FALSE}
phat_RF <- predict(rf.npf, newdata = npf_valid.rf)

kable(table(ifelse(phat_RF >= 0.5, 1, 0), npf_valid$class2))
```

There are ${20}$ observations predicted to be nonevent even though event has occurred and respectively ${15}$ observations given the false prediction of event. All together, the accuracy in validation set is ${84,9 \%}$.
...
l.acc.RF_CV = NULL
l.perp.RF_CV = NULL
npf.all.clean.rf <- data.frame(scale(npf.all.clean[,-1]),class2=class2_num_all)

for (l in seq(CV.rep)){
  split = split.CV(npf.all.clean.rf,10)
  phat_RF_CV <- c()
  for (i in 1:10){
    ind.test = split[[i]]
    ind.all = unlist(split)
    ...
    # x <- model.matrix(class2 ~., npf.all.clean[ind.train,])[,-1]
    # y <- npf.all.clean$class2[ind.train]
    rf.npf_CV <- randomForest(class2~., data = npf.all.clean.rf[ind.train,], mtry = 9, importance = TRUE)
    phat_RF_CV<- predict(rf.npf_CV, newdata = npf.all.clean.rf[ind.test,])
    # x.test <- model.matrix(class2 ~., npf.all.clean[ind.test,])[,-1]
  ...
  l.perp.RF_CV = c(l.perp.RF_CV,perplexity(phat_RF_CV, npf.all.clean.rf$class2))
}
...
```
The picture above report the values of two variables:

  - Mean Decrease Accuracy (%IncMSE) that shows how much our model accuracy decreases if we leave out that variable.
  
  - Mean Decrease in MSE (IncNodePurity) that is a measure of variable importance based on the MSE.
...
Naive Bayes with all ${26}$ variables performs poorly producing high perplexities. When the dimensionality is reduced by using PCA the performance improves significantly but does not outperform logistic regression models. The ${10}$-NN is close to the performance of Naive Bayes with PCA.
...
After selecting the model we trained it using the original, cleaned training data (npf_train.csv). The estimates of the probabilities of the observations in testing data (npf_hidden.csv) to be classified as "event" were predicted. If the probability is more than ${0.5}$ the observation is labeled as "event" and otherwise "nonevent".

```{r, message=FALSE, echo=FALSE,warning=T, eval = FALSE}
# CHALLENGE 11.12.2022
#The type of the event was set to be the same type that occurs mostly in the training data. The table below shows the frequencies of different event types in the training data when the event has happened. The type "II" has the highest frequency of 48,7 %
...
The accuracy and perplexity on testing data with 965 observations are

```{r, message=FALSE, echo=FALSE,warning=T}
set.seed(42)
...
#accuracy and perplexity on testing data calculated after the end of challenge
class2_num_test <- c(ifelse(npf_hidden_clean$class2=="event",1,0))
acc_test <- round(accuracy(p, class2_num_test),3)
perp_test <- round(perplexity(p, npf_hidden_clean$class2),3)
performance <- data.frame(acc_test, perp_test)
colnames(performance)<- c("accuracy", "perplexity")
kable(performance)
...
#Code for forming the answers.csv to the challenge 11.12.2022
...
Compared to the performance in validation and cross-validation approaches, the performance has reduced significantly. The performance indicates that the method is guessing if the event will happen or not with no prediction power. With this performance we should check the data process and feature selections again and try some other methods, too. For example, support vector machine with radial kernel (gamma = 0.5, cost = 0.1) gives promising performance measures on testing data: 
...
svm <- svm(class2~., data = npf.all.clean.rf, kernel = "radial", gamma = g, cost = c, scale = TRUE)
pred_svm <- predict(svm, npf_test.scaled)

svm.acc_test <- round(accuracy(pred_svm, class2_num_test),3)
svm.perp_test <- round(perplexity(pred_svm, class2_num_test),3)

performance_svm <- data.frame(svm.acc_test, svm.perp_test)
colnames(performance_svm)<- c("accuracy", "perplexity")
...
#Random forest with selected variables (unscaled)
set.seed(42)
sel <- c("Glob.mean","RPAR.mean", "RHIRGA168.std", "PTG.std", "CO2168.mean")
count <- c(2)
sel
npf_test.sel <- data.frame(npf_hidden_clean[,sel], class2 = class2_num_test)
npf_train.sel <- data.frame(npf.all.clean[, sel], class2 = class2_num_all)

rf.npf_test.sel <- randomForest(class2~., data = npf_train.sel, mtry = count, importance = TRUE)
phat_RF_test <- predict(rf.npf_test.sel, newdata = npf_test.sel)

acc_test_rf<-round(accuracy(phat_RF_test, class2_num_test),3) 
perp_test_rf <-round(perplexity(phat_RF_test, class2_num_test),3)

performance_rf <- data.frame(acc_test_rf, perp_test_rf)
colnames(performance_rf)<- c("accuracy", "perplexity")
kable(performance_rf)
```
...
set.seed(42)
npf_test.rf <- data.frame(scale(npf.test.clean),class2=class2_num_test)
rf.npf_test <- randomForest(class2~., data = npf.all.clean.rf, mtry = 9, importance = TRUE)
phat_RF_test <- predict(rf.npf_test, newdata = npf_test.rf)

acc_test_rf<-round(accuracy(phat_RF_test, class2_num_test),3) 
perp_test_rf <-round(perplexity(phat_RF_test, npf_hidden_clean$class2),3)
...
set.seed(42)
k_sel <- c(10)
train.X <- scale(npf.all.clean[,-1])
test.X <- scale(npf.test.clean)
train.Y <- npf.all.clean$class2
test.Y <- npf_hidden_clean$class2

#Testing
knn.Y <- knn(train.X, test.X, train.Y, k=k_sel,prob = T)
p.knn = attributes(knn.Y)$prob
ind.no_test = which(knn.Y=="nonevent")
p.knn[ind.no_test] = 1-p.knn[ind.no_test]

knn.Y_num <- ifelse(knn.Y=="event",1,0)
knn.acc_test <- round(mean(ifelse(knn.Y_num==class2_num_test,1,0)),3)
knn.perp_test <- round(perplexity(p.knn,test.Y),3)

performance_knn <- data.frame(knn.acc_test, knn.perp_test)
colnames(performance_knn)<- c("accuracy", "perplexity")
kable(performance_knn)
```


```{r, message=FALSE,warning=FALSE, echo=FALSE, eval=FALSE}
#Logistic regression (scaled data)
npf_test.scaled <- data.frame(test.X,class2=npf_hidden_clean$class2)

phat_LG_test <- function(npf.all.clean.scaled,
  npf_test.scaled,
  model = function(data) glm(class2 ~ ., data, family = "binomial"),
  pred = function(model, x) predict(model, newdata = x, type = "response")) {
    m <- model(npf.all.clean.scaled)
    pred(m, npf_test.scaled)
}

#Test
phat_LG_test <- phat_LG_test(npf.all.clean.scaled, npf_test.scaled)

LG.acc_test <- round(accuracy(phat_LG_test, class2_num_test),3)
LG.perp_test <- round(perplexity(phat_LG_test, test.Y),3)

performance_LG <- data.frame(LG.acc_test, LG.perp_test)
colnames(performance_LG)<- c("accuracy", "perplexity")
kable(performance_LG)
```


```{r, message=FALSE,warning=FALSE, echo=FALSE, eval=FALSE}
#Logistic regression using only the selected variables (unscaled)
sel <- c("class2", "Glob.mean","RPAR.mean", "RHIRGA168.std", "PTG.std")
sel
npf_test.sel <- npf_hidden_clean[,sel]
npf_train.sel <- npf.all.clean[, sel]

phat_LG_test <- function(npf_train.sel,
  npf_test.sel,
  model = function(data) glm(class2 ~ ., data, family = "binomial"),
  pred = function(model, x) predict(model, newdata = x, type = "response")) {
    m <- model(npf_train.sel)
    pred(m, npf_test.sel)
}

#Test
phat_LG_test <- phat_LG_test(npf_train.sel, npf_test.sel)

LG.acc_test <- round(accuracy(phat_LG_test, class2_num_test),3)
LG.perp_test <- round(perplexity(phat_LG_test, npf_hidden_clean$class2),3)
