...
kable(knn.acc.table)
```
When k = 5 or 10, the accuracy is highest. We add 10-NN as one of the methods since with a higher k the model is not so flexible and we can avoid overfitting.
...
As we already concluded logistic regression with Lasso performs better than random forest. Logistic regression with all 26 variables performs surprisingly well but not as well as when variable selection is done with Lasso. 
