...
for (l in seq(CV.rep)){
  ...
  for (i in 1:10){  
    ...
    # knn.Y_CV[ind.test] <- knn(train.X, valid.X, train.Y, k=k_sel) #remove
    knn.Y_CV <- knn(train.X, valid.X, train.Y, k=k_sel)
    knn.acc_CV <- mean(knn.Y_CV == valid.Y)
    
    # knn.Y_CV_num <- ifelse(knn.Y_CV=="event",1,0)
    # 
    # # knn.acc_CV <- round(mean(ifelse(knn.Y_CV_num==class2_num_all,1,0)),3) # remove
    # knn.acc_CV <- mean(ifelse(knn.Y_CV_num==class2_num_all[ind.test],1,0))
  }
