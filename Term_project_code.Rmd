...
perplexity <- function(p, y) exp(-mean(log(ifelse(y == "event", 
                                                  ifelse(p==0,1*10^-100,p),
                                                  ifelse(p==1,1*10^-100,1 - p)))))
...
x.ans.test <- model.matrix(~., npf_hidden_clean)[,-1] #hidden data does not include class2 so this is not working...?
